...
def calculate_concentration_loss(attention_maps, indices_to_alter):
    
    print(f"attention_maps: {len(attention_maps)}{attention_maps[0].shape}, indices_to_alter: {indices_to_alter}")
    indices_to_alter = [indices_to_alter] if not isinstance(indices_to_alter, list) else  indices_to_alter

    attn_map = torch.stack(
        [
            attention_maps[s]
            for s in indices_to_alter
        ]
        , dim = 0
    )
...
def calculate_concentration_loss_syngen(attention_maps, modifier, noun):

    print(f"attention_maps: {len(attention_maps)}{attention_maps[0].shape}, modifier: {modifier}., noun: {noun}")
